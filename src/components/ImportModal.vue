...
import { parseX4DataFolder } from '../utils/x4XmlImport'
...
const submitSourceLabel = computed(() => {
  ...
  if (parsed.value && parsedMode.value === 'x4-folder') return '自动识别数据'
  if (parsed.value) return '刚选择的整理文件'
  return '当前本地导入'
})

async function readModDataFolder(event) {
  const files = [...(event.target.files || [])]
  event.target.value = ''
  if (!files.length) return
  status.value = '正在扫描 mod 数据文件夹...'
  try {
    const result = await parseX4DataFolder(files)
    parsed.value = result.datasets
    parsedMode.value = 'x4-folder'
    scanInfo.value = result
    rawFiles.value = []
    rawLinks.value = [{ label: '', url: '', access_code: '', size_hint: '', sha256: '', note: '' }]
    if (!submit.packageName) submit.packageName = inferFolderPackageName(files)
    const counts = Object.entries(result.report.counts).map(([key, value]) => `${datasetLabel(key)} ${value}`).join('、')
    const warnings = result.warnings.length ? `；提示：${result.warnings.join('；')}` : ''
    status.value = `已扫描 ${result.scanned} 个数据文件，识别 ${result.report.total} 条：${counts || '暂无可提交条目'}${warnings}`
  } catch (err) {
    parsed.value = null
    parsedMode.value = ''
    scanInfo.value = null
    status.value = `自动识别失败：${err.message}`
  }
}
...
function inferFolderPackageName(files) {
  const path = files[0]?.webkitRelativePath || files[0]?.name || ''
  return path.includes('/') ? path.split('/')[0] : '自动识别 mod 数据'
}

function datasetLabel(key) {
  return DATASETS[key]?.label || key
}
  ...
  <section class="modal" :class="{ open }">
    ...
    <div class="modal-body import-body">
      <section class="import-block import-fast">
        <h4>自动识别 mod 数据文件夹</h4>
        <p>推荐选择你的 mod 根目录，或选择只包含 assets / libraries / t 的数据文件夹。网页只读取 XML、CSV、JSON 和文本数据，自动忽略模型、贴图、音频等大文件。</p>
        <div class="button-row">
          <label class="btn primary file-btn">
            选择 mod 数据文件夹
            <input type="file" multiple webkitdirectory directory @change="readModDataFolder" />
          </label>
          <button type="button" class="btn" :disabled="!parsed" @click="confirmImport">先合并到本地预览</button>
          <button type="button" class="btn primary" :disabled="!parsed" @click="submitForReview">提交识别结果审核</button>
        </div>
        <div class="quick-submit-row">
          <span>{{ scanInfo ? `扫描 ${scanInfo.scanned} 个数据文件，识别 ${scanInfo.report.total} 条` : '自动识别舰船、武器、炮塔、装备、弹体、ware、建造材料和文本名' }}</span>
        </div>
        <div v-if="scanInfo?.warnings?.length" class="scan-warnings">
          <small v-for="warning in scanInfo.warnings" :key="warning">{{ warning }}</small>
        </div>
      </section>
